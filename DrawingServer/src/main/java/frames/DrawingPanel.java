...
public class DrawingPanel extends JPanel implements java.awt.print.Printable {
  ...
  private void finishTransforming(int x2, int y2) {
    ...
    castShape(this.selectedShape);

    this.repaint();
  }

  private void castShape(GShape shape) {
    CMInteractionInfo interInfo = CMPaintApp.m_clientStub.getCMInfo().getInteractionInfo();
    ...
    String serializedShape = serializeToString(shape);
    due.setDummyInfo(serializedShape);

    CMPaintApp.m_clientStub.cast(due, myself.getCurrentSession(), myself.getCurrentGroup());
    System.out.println(serializedShape);
    due = null;
  }
